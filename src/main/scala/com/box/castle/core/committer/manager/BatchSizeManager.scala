...
/**
  ...
  *
  */
class BatchSizeManager(committerActorId: String,
  ...
  def getDelay: Duration = {
    ...
    if (samples.size > 1) {
      ...
      log.info(s"$committerActorId idling for ${computedDelay / 1000} seconds after calculating a rate of ${rate / 1000} KiB/sec with discountFactor of $discount")
